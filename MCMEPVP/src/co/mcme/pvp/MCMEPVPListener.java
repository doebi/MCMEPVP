...
public class MCMEPVPListener implements Listener {
        ...
        if (MCMEPVP.GameStatus == 1) {
            ...
            if (Team.equals("spectator")) {
                event.setDeathMessage(ChatColor.YELLOW 
                        + "Spectator " 
                        + player.getName() 
                        + " was tired watching this fight!");
            }
            if (Team.equals("red")) {
                event.setDeathMessage(ChatColor.RED 
                        + "Team Red " 
                        + ChatColor.YELLOW 
                        + "lost " 
                        + player.getName());
    ...
    void onPlayerDamage(final EntityDamageByEntityEvent event) {
        if (event.getDamager().getType().equals(EntityType.PLAYER) 
            ...
            if (!(AttackerTeam.equals("spectator"))
                    && !(VictimTeam.equals("spectator"))
                    && !(AttackerTeam.equals("participant"))
                    && !(VictimTeam.equals("participant"))
                ...
                Attacker.sendMessage(ChatColor.DARK_RED 
                        + "You can't attack " 
                        + Victim.getName() + "!");
                event.setCancelled(true);
            }
